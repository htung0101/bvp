...
if __name__ == "__main__":
    ...
    file_id = args.fileid #"0"
    support_fileid = ["0"]
    assert(file_id in support_fileid)
    ...
    filename = os.path.join(mark_data_dir, filenames[file_id])

    data = h5py.File(filename)["data"]
    rgb_path = os.path.join(mark_data_dir, rgb_folder[file_id])
    ...
    for trim_id, step_id in enumerate(trim_dict[file_id][:-1]):
        ...
        #create_gif(os.path.join(args.output_dir, f'rgbs_{file_id}_step{step_id}.gif'), rgbs)  
        #create_gif(os.path.join(args.output_dir, f'depths_{file_id}_step{step_id}.gif'), depths)
        create_gif(os.path.join(args.output_dir, f'rgbd_{file_id}_step{step_id}.gif'), np.concatenate([rgbs, depths], axis=2))
        #fig = plt.figure()
        #ims = []
